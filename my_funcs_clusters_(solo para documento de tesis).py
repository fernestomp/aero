# Notebook de funciones que usan los demas notebooks. Este debe contener las fuciones actualizadas
# %% md
...
def raw_to_datafr(xlsPath, xlsPathMfgCurve):
    ...
    dataVPxls = pd.read_excel(xlsPath, usecols=[0, 1, 2],
                              index_col=0)
    ...
    dfMfgCurve = pd.read_excel(xlsPathMfgCurve,
    ...
    print(datamk.loc[datamk.isnull().any(axis=1),
          :].isnull().sum())
    ...
    dataVP['Pw'] = (cleanData.iloc[1:,
                    1].values - cleanData.iloc[0:-1,
                                1]) * np.pi * 45 ** 2
    ...
    # Eliminando outliers
    dataVP.drop([pd.to_datetime('2016-03-07 09:50:00')],
                inplace=True)
    # agregado para el reporte
    dataVP.drop([pd.to_datetime('2016-03-08 09:00:00')],
                inplace=True)
    dataVP.drop([pd.to_datetime('2016-01-02 06:40:00')],
                inplace=True)
    dataVP.drop([pd.to_datetime('2016-05-05 07:20:00')],
                inplace=True)
    dataVP.drop([pd.to_datetime('2016-05-23 23:00:00')],
                inplace=True)

    # leyendo la direccion del viento
    # dataDirVxls = pd.read_excel(xlsPath,sheet_name=1, usecols=[0,1],index_col=0,names=['Dir'])
    dataDirVxls = pd.read_excel(xlsPath, sheet_name=1,
                                usecols=[0, 1], index_col=0)
    dataDirVxls.columns = ['Dir']
    # limpiando datos
    # marcando los datos faltantes asignando un nan a la fila completa
    datamkdir = dataDirVxls
    datamkdir.loc[datamkdir.isnull().any(axis=1),
    :] = np.nan
    # eliminando filas con NaN. Si busco la fecha anterior debe aparecer error.
    ...
    dataDir.drop([pd.to_datetime('2016-03-07 09:50:00')],
                 inplace=True)
    # agregado para el reporte
    dataDir.drop([pd.to_datetime('2016-03-08 09:00:00')],
                 inplace=True)
    dataDir.drop([pd.to_datetime('2016-01-02 06:40:00')],
                 inplace=True)
    dataDir.drop([pd.to_datetime('2016-05-05 07:20:00')],
                 inplace=True)
    dataDir.drop([pd.to_datetime('2016-05-23 23:00:00')],
                 inplace=True)

    direcvrad = np.deg2rad(dataDir['Dir'].values)
    velocidades = dataVP.iloc[:]['vViento'].values
    vecVel = [-np.sin(direcvrad) * velocidades,
              np.cos(direcvrad) * velocidades]
    vecVelnp = np.array(vecVel).transpose()
    # original sin timestamp
    # dfVecVel = pd.DataFrame(data=vecVelnp,columns=['vx','vy']
    # con timestamp
    dfVecVel = pd.DataFrame(data=vecVelnp,
                            columns=['vx', 'vy'],
                            index=dataVP.index)
    ...
    dataVDP = pd.concat(
        [dataVP.vViento, dataDir, dataVP.Pw], axis=1)
    # dataVcD =pd.concat([dataDir,dfVecVel],axis=1)
    # Datos en p.u.. Divido todos los datos entre el maximo del conjunto de datos
...
def dataframe_to_cluster(dfvxvy, n_clusters, dfVP=None,
                         n_subclu=0, clusters_data=None,
                         subclusters_data=None,
                         datadir=None):
    """
        Descripcion:
        ...
        :param n_subclu: Numero de suclusters a calcular a partir de los n clusters calculados en un principio.
                  Si n_subclu=0 no se calcula ningun subcluster. Por defecto n_sub=0


        :return cl_ord: un array que contiene el numero de cluster n ordenado de menor a mayor
    ...
    if clusters_data == 'wind':
        ...
        min_dist = np.min(
            cdist(dfvxvy.values, kmeans.cluster_centers_,
                  'euclidean'),
            axis=1)  # distancia minima a cada centroide
        Y = pd.DataFrame(min_dist, index=dfvxvy.index,
                         columns=['PCTimeStamp'])
        Z = pd.DataFrame(kmeans.labels_, index=dfvxvy.index,
                         columns=['cluster_ID'])
        # crea el dataframe
    ...
    if clusters_data == 'pow' and not dfvp_is_empty:
        ...
        kmeans = cluster.KMeans(n_clusters=n_clusters,
                                random_state=0,
                                init='k-means++',
                                n_init='auto').fit(dfVP)
        # buscar centroides en dataframes
        min_dist = np.min(
            cdist(dfVP.values, kmeans.cluster_centers_,
                  'euclidean'), axis=1)
        Y = pd.DataFrame(min_dist, index=dfVP.index,
                         columns=['PCTimeStamp'])
        Z = pd.DataFrame(kmeans.labels_, index=dfVP.index,
                         columns=['cluster_ID'])
        PAP = pd.concat([Y, Z], axis=1)
    ...
    if clusters_data == 'dir' and not dfvp_is_empty:
        ...
        print(
            'Clustering with only direction and no power data not implemented yet.')
        return None
    ...
    for i in range(n_clusters):
        ...
        cl_magni[i] = np.mean(
            np.sqrt(vx ** 2 + vy ** 2))  # magnitud de la vv
    cl_ord = np.argsort(
        cl_magni.argsort())  # ver https://github.com/numpy/numpy/issues/8757

    grouped = PAP.groupby([
        'cluster_ID'])  # agrupa por numero de clusters las distancias minimas
    ...
    if dfvp_is_empty:
        idx_centroids = idx_centroids.assign(vx=vxvy[:, 0],
                                             vy=vxvy[:, 1])
    else:
        ...
        idx_centroids = idx_centroids.assign(vx=vxvy[:, 0],
                                             ...
                                             vwind=vvpot[:,
                                                   0],
                                             pw=vvpot[:, 1])
        idx_centroids.sort_values(by='vwind',
                                  inplace=True)  # ordenar por velocidad de viento

    idx_centroids.reset_index(
        inplace=True)  # que ya no sea el cluster id el indice
    idx_centroids.index.set_names('cluster_ID_ord',
                                  inplace=True)  # ponerle el nombre al indice ordenado
    ...
    if dfvp_is_empty:
        ...
        for i in range(n_clusters):
            dfclvv = pd.concat([dfclvv, dfvxvy.vx[
                kmeans.labels_ == np.where(cl_ord == i)[0][
                    0]], dfvxvy.vy[kmeans.labels_ ==
                                   np.where(cl_ord == i)[0][
                                       0]]], axis=1,
                               ignore_index=True)
        # crear multiindice
        dfclvv.columns = pd.MultiIndex.from_tuples(
            colheadvv)
        ...
        dfclpw = pd.DataFrame()
        for i in range(n_clusters):
            dfclpw = pd.concat([dfclpw, dfVP.Pw[
                kmeans.labels_ == np.where(cl_ord == i)[0][
                    0]]], ignore_index=True, axis=1)
        ...
        for i in range(n_clusters):
            dfclvp = pd.concat([dfclvp, dfVP.vViento[
                kmeans.labels_ == np.where(cl_ord == i)[0][
                    0]], dfVP.Pw[kmeans.labels_ ==
                                 np.where(cl_ord == i)[0][
                                     0]]], axis=1,
                               ignore_index=True)
        # crear multiindice
        dfclvp.columns = pd.MultiIndex.from_tuples(
            colheadvp)
    ...
    if clusters_data == 'direccion':
        ...
        dfcldv = pd.DataFrame()
        for i in range(n_clusters):
            dfcldv = pd.concat([dfcldv, datadir.vViento[
                kmeans.labels_ == (cl_ord == i)],
                                datadir.Dir[
                                    kmeans.labels_ == (
                                                cl_ord == i)]],
                axis=1, ignore_index=True)
        # crear multiindice
        dfcldv.columns = pd.MultiIndex.from_tuples(
            colheadvp)
    ...
    if n_subclu > 0:
        ...
        for i in range(n_clusters):
            if subclusters_data == 'wind':
                ...
                kmeans_sc = cluster.KMeans(
                    n_clusters=n_subclu, random_state=0,
                    init='k-means++', n_init='auto').fit(
                    dfclvvnoNA)
                min_dist_sc = np.min(
                    cdist(dfclvvnoNA.values,
                          kmeans_sc.cluster_centers_,
                          'euclidean'), axis=1)
                Y_sc = pd.DataFrame(min_dist_sc,
                                    index=dfclvvnoNA.index,
                                    columns=['PCTimeStamp'])
                Z_sc = pd.DataFrame(kmeans_sc.labels_,
                                    index=dfclvvnoNA.index,
                                    columns=[
                                        'subcluster_ID'])
                ...
                kmeans_sc = cluster.KMeans(
                    ...
                    dfclvpnoNA)
                min_dist_sc = np.min(
                    cdist(dfclvpnoNA.values,
                          kmeans_sc.cluster_centers_,
                          'euclidean'), axis=1)
                Y_sc = pd.DataFrame(min_dist_sc,
                                    index=dfclvpnoNA.index,
                                    columns=['PCTimeStamp'])
                Z_sc = pd.DataFrame(kmeans_sc.labels_,
                                    index=dfclvpnoNA.index,
                                    columns=[
                ...
                print(
                    'Todavia no programado, haciendo subclusters en potencia')
                kmeans_sc = cluster.KMeans(
                    n_clusters=n_subclu, random_state=0,
                    init='k-means++', n_init='auto').fit(
            ...
            # agregando columnas de velocidadd de viento y potencia al dataframe de los centroides
            vxvy_sc = dfvxvy.loc[
                idx_cent_sc.PCTimeStamp].values
            vvpot_sc = dfVP.loc[
                idx_cent_sc.PCTimeStamp].values
        ...
        idx_centroids_sc.sort_values(
            ['cluster_ID', 'vwind'], ascending=[1, 1],
            inplace=True)
        lst_num_sc = list(range(1,
                                n_subclu + 1)) * n_clusters  # crea una lista de numeros para el subcluster [1,2,3..1,2,3]
        sc_idx_list = list(map(lambda x: 'SC' + str(x),
                               lst_num_sc))  # le pone las letras SC [SC1,SC2,SC3...SC1,SC2,SC3]
        idx_centroids_sc = idx_centroids_sc.assign(
            subcluster_ID_ord=sc_idx_list)
        idx_centroids_sc.set_index(
        ...
        for i in range(n_clusters):
            ...
            for j in range(n_subclu):
                vx = \
                dfclvv['C' + str(i + 1)].vx.dropna().values[
                    scl_labels[i] == j]
                vy = \
                dfclvv['C' + str(i + 1)].vy.dropna().values[
                    scl_labels[i] == j]
                scl_magni[i][j] = np.mean(np.sqrt(
                    vx ** 2 + vy ** 2))  # magnitud de la vv

            scl_ord.append(scl_magni[
                               i].argsort())  # ver https://github.com/numpy/numpy/issues/8757
        ...
        for i in range(n_clusters):
            for j in range(n_subclu):
                colheadvp.append((
                'C' + str(i + 1), 'SC' + str(j + 1),
                'vViento'))
                colheadvp.append(('C' + str(i + 1),
                                  'SC' + str(j + 1), 'Pw'))
        dfsclvp = pd.DataFrame()

        for i in range(n_clusters):
            for j in range(n_subclu):
                dfsclvp = pd.concat([dfsclvp, dfclvp[
                    'C' + str(i + 1)].dropna().vViento[
                    scl_labels[i] == scl_ord[i][j]], dfclvp[
                                         'C' + str(
                                             i + 1)].dropna().Pw[
                                         scl_labels[i] ==
                                         scl_ord[i][j]]],
                                    ...
                                    ignore_index=True)
        # crear multiindice
        dfsclvp.columns = pd.MultiIndex.from_tuples(
            colheadvp)
        ...
        for i in range(n_clusters):
            for j in range(n_subclu):
                dfsclvv = pd.concat([dfsclvv, dfclvv[
                    'C' + str(i + 1)].dropna().vx[
                    scl_labels[i] == scl_ord[i][j]], dfclvv[
                                         'C' + str(
                                             i + 1)].dropna().vy[
                                         scl_labels[i] ==
                                         scl_ord[i][j]]],
                                    ...
                                    ignore_index=True)
        # crear multiindice
        dfsclvv.columns = pd.MultiIndex.from_tuples(
            colheadvv)
        ...
        for i in range(n_clusters):
            for j in range(n_subclu):
                dfsclpw = pd.concat([dfsclpw, dfclvp[
                    'C' + str(i + 1)].Pw.dropna()[
                    scl_labels[i] == scl_ord[i][j]]],
                                    ignore_index=True,
                                    axis=1)
        dfsclpw.columns = pd.MultiIndex.from_tuples(
            colheadpw)
        ...
        namcl = [(lev0[i], lev1[i]) for i in
                 range(len(lev0))]
        colnames = namcl[::2]

        return (dfsclvv, dfsclpw, dfsclvp, cl_ord,
                kmeans.cluster_centers_, idx_centroids,
                scl_ord, scl_centroids, idx_centroids_sc)
        ...
        if dfvp_is_empty:
            return (
            dfclvv, [], [], cl_ord, kmeans.cluster_centers_,
            idx_centroids, scl_ord, scl_centroids,
            idx_centroids_sc)
        else:
            return (dfclvv, dfclpw, dfclvp, cl_ord,
                    kmeans.cluster_centers_, idx_centroids,
                    scl_ord, scl_centroids,
                    idx_centroids_sc)
...
def del_clusters(data, cltodel, idx_cent, idx_cent_sc=None,
                 scl_ord=None, cl_type='cluster'):
    """
    ...
    c = (b - a)  # a b lequito a
    cent_restantes = set(sorted(set(
        int(item[0][1:]) for item in
        c)))  # quito la letra C
...
def clust_to_data(dfv, dfvp):
    ...
    # list comprehension
    lvv = [dfv.iloc[i].dropna().values for i in
           range(len(dfv))]
    dfvxvy_clean = pd.DataFrame(lvv, columns=['PCTimeStamp',
                                              'vx', 'vy'])
    dfvxvy_clean.dropna(
        inplace=True)  # hay filas de nan y se pierden miles de datos, revisar

    # Quitar multiindex
    dfvp.columns = dfvp.columns.map(''.join)
    dfvp.reset_index(inplace=True)
    # list comprehension
    lvp = [dfvp.iloc[i].dropna().values for i in
           range(len(dfvp))]
    dfvp_clean = pd.DataFrame(lvp, columns=['PCTimeStamp',
                                            'vViento',
                                            'Pw'])
    dfvp_clean.dropna(
        inplace=True)  # hay filas de nan y se pierden miles de datos, revisar
...
class KMData:
    ...
    # crear clusters a partir de dataframes
    def dataframe_to_cluster(self, dfvxvy, n_clusters,
                             dfVP=None, n_sub_clusters=0,
                             clusters_data=None,
                             subclusters_data=None,
                             datadir=None, pw_col_name='pw',
                             wind_col_name='vwind'):
        """
            Descripcion:
            Esta funcion toma diferentes dataframes de entrada agrupa los datos por cluster.
            ...
            :param dfVP: dataframe con columnas de magnitud de viento y potencia
            :param n_clusters: numero de clusters
            :param n_sub_clusters: Numero de suclusters a calcular a partir de los n clusters calculados en un principio.
        ...
        if clusters_data == 'wind':
            ...
            # buscar centroides en dataframes
            # https://stackoverflow.com/questions/42583995/get-the-centroid-row-index-from-k-means-clustering-using-sklearn
            min_dist = np.min(cdist(dfvxvy.values,
                                    kmeans.cluster_centers_,
                                    'euclidean'),
                axis=1)  # distancia minima a cada centroide
            Y = pd.DataFrame(min_dist, index=dfvxvy.index,
                             columns=['PCTimeStamp'])
            Z = pd.DataFrame(kmeans.labels_,
                             index=dfvxvy.index,
                             columns=['cluster_ID'])
            # crea el dataframe
            PAP = pd.concat([Y, Z], axis=1)
        ...
        vxvy = dfvxvy.loc[idx_centroids.PCTimeStamp].values
        if dfvp_is_empty:
            idx_centroids = idx_centroids.assign(
                vx=vxvy[:, 0], vy=vxvy[:, 1])

        else:
            vvpot = dfVP.loc[
                idx_centroids.PCTimeStamp].values
            idx_centroids = idx_centroids.assign(
                vx=vxvy[:, 0], vy=vxvy[:, 1],
                vwind=vvpot[:, 0], pw=vvpot[:,
        ...
        if dfvp_is_empty:
            ...
            for i in range(n_clusters):
                dfclvv = pd.concat([dfclvv, dfvxvy.vx[
                    kmeans.labels_ ==
                    np.where(cl_ord == i)[0][0]], dfvxvy.vy[
                                        kmeans.labels_ ==
                                        np.where(
                                            cl_ord == i)[0][
                                            0]]], axis=1,
        ...
        if clusters_data == 'direccion':
            ...
            for i in range(n_clusters):
                colheadvp.append(('C' + str(i + 1), 'Dir'))
                colheadvp.append(
                    ('C' + str(i + 1), pw_col_name))
            dfcldv = pd.DataFrame()
            for i in range(n_clusters):
                dfcldv = pd.concat([dfcldv,
                                    datadir[wind_col_name][
                                        kmeans.labels_ == (
                                                    cl_ord == i)],
                                    datadir.Dir[
                                        kmeans.labels_ == (
        ...
        if n_sub_clusters > 0:
            ...
            for i in range(n_clusters):
                ...
                vvpot_sc = dfVP.loc[
                    idx_cent_sc.PCTimeStamp].values
                idx_centroids_sc = pd.concat(
                    [idx_centroids_sc,
                     idx_cent_sc.assign(vx=vxvy_sc[:, 0],
                         vy=vxvy_sc[:, 1],
                         vwind=vvpot_sc[:, 0],
                         pw=vvpot_sc[:, 1],
            ...
            for i in range(n_clusters):
                ...
                for j in range(n_sub_clusters):
                    vx = dfclvv['C' + str(
                        i + 1)].vx.dropna().values[
                        scl_labels[i] == j]
                    vy = dfclvv['C' + str(
                        i + 1)].vy.dropna().values[
                        scl_labels[i] == j]
                    scl_magni[i][j] = np.mean(np.sqrt(
                        vx ** 2 + vy ** 2))  # magnitud de la vv
            ...
            # ORDENAR CENTROIDES
            for i in range(len(scl_centroids)):
                scl_centroids[i] = scl_centroids[i][
                    scl_ord[i]]
...
# %%
'''
Se reemplaza la hora 24:00 por 00:00 y se suma un dia, asi que la ultima
toma de datos de un dia se cambia por la primera del dia siguiente
'''


def proc_dat_bcs(data_path, mf_pow_curve_path=None,
                 mf_vel_colname=None, mf_pw_colname=None,
                 mf_rename_cols=None):
    """
    ...
    dataVDxls = pd.read_excel(data_path,
    ...
    dataVDxls['hour'] = dataVDxls['hour'].str.replace(
        '2400', '0000')
    dataVDxls['timeStamp'] = dataVDxls.apply(
        lambda x: daymin2date(year, x.day, x.hour), axis=1)
    # dataVDxls['timeStamp'] =pd.to_datetime(pd.to_datetime(dataVDxls['timeStamp'], format='%d/%m/%Y '
    #                                                                                     '%H:%M').dt.strftime(
    #                                                                                     '%d/%m/%Y %H:%M'))
    dataVDxls['timeStamp'] = pd.to_datetime(
        dataVDxls['timeStamp'], format='%d/%m/%Y %H:%M')

    # cambiar formato de 24:00 a 00:00 sumando un dia a 00:00
    ...
    dataVDxls.set_index('timeStamp', inplace=True,
    ...
    if dir_format == 'rad':
        ...
        vecVel = [-np.sin(direcvrad) * dfWD['vwind'].values,
                  np.cos(direcvrad) * dfWD['vwind'].values]
    else:
        vecVel = [
            -np.sin(dfWD['wdir'].values * np.pi / 180) *
            dfWD['vwind'].values,
            np.cos(dfWD['wdir'].values * np.pi / 180) *
            dfWD['vwind'].values]
    ...
    # con timestamp
    df_comp_vel = pd.DataFrame(data=vecVelnp,
                               columns=['vx', 'vy'],
                               index=dfWD.index)
    ...
    if mf_pow_curve_path is not None:
        if mf_pow_curve_path[-3:] == 'csv':
            ...
            dfMfgCurve = pd.read_excel(mf_pow_curve_path,
                                       usecols=[0, 2],
                                       index_col=0)
        if mf_rename_cols is not None:
            dfMfgCurve.rename(
                columns={mf_vel_colname: mf_rename_cols[0],
                         mf_pw_colname: mf_rename_cols[1]},
                inplace=True)
            dfMfgCurve.set_index(mf_rename_cols[0],
                                 inplace=True)
...
def interp_pow_curve(df_mf_curve, wind_min_max=(0, 25),
                     step=0.1):
    """
    ...
    wind_range = list(np.round(
        np.arange(wind_min_max[0], wind_min_max[1], step),
        decimals=1))
    df_mfcurve_interp = pd.DataFrame(data={'pw': np.nan},
                                     index=wind_range)
    ...
    for i in df_mf_curve.index:
        df_mfcurve_interp[df_mfcurve_interp.index == i] = \
        df_mf_curve.loc[i].pw
    # interpolando
    df_mfcurve_interp.interpolate(method='linear',
                                  inplace=True)
...
#
#     :param fig_size: tamanio de la figura que se muestra en el notebook.
#     :param trans_matrix: dataframe que contiene la matriz de transicion de probabilidades.
#     :param marker_size: tamanio del nodo (punto que representa al estado)
#     :param node_fontsize: tamanio de la fuente.
#     :param line_width: tamanio de linea.
#     :param x_width: ancho de la imagen en pulgadas.
#     :param y_width: largo de la imagen en pulgadas.
#     """
#     #tamanio del nodo (punto)
...
def plot_vectors(comp_vel, show_month=None, figsize=(7, 5),
    ...
    limix = [np.floor(comp_vel.vx.min()),
             np.ceil((comp_vel.vx.max()))]
    limiy = [np.floor(comp_vel.vy.min()),
             np.ceil((comp_vel.vy.max()))]
    limir = np.ceil(np.max(comp_vel.max()))
    if show_month is None:
        # mostrar todos los meses en un solo plot
        origin = np.array([np.zeros(len(comp_vel)),
                           np.zeros(
                               len(comp_vel))])  # origin point
        plt.suptitle('All months')
        plt.quiver(*origin, comp_vel.vx, comp_vel.vy,
        ...
        for i in range(3):
            for j in range(4):
                # slicing mes por mes
                data_month = comp_vel[
                    comp_vel.index.month == i + j + 1]
                origin = np.array(
                    [np.zeros(len(data_month)), np.zeros(
                        len(data_month))])  # origin point
                ax[i, j].quiver(*origin, data_month.vx,
                                data_month.vy, color='b',
                                angles='xy',
                                scale_units='xy', scale=1)
                # ax[i,j].title.set_text(months[i+j+1])
                ax[i, j].title.set_text(
                    months[counter_month])
        ...
        data_month = comp_vel[
            comp_vel.index.month == show_month]
        origin = np.array([np.zeros(len(data_month)),
                           np.zeros(
                               len(data_month))])  # origin point
        ...
        print('Error. Parametro show_month no valido.')
    plt.figtext(0.5, 0.01, 'Figure ' + str(fign), wrap=True,
...
def join_clusters(df, clu_to_join):
    ...
    if df.columns.nlevels == 1:
        ...
        df_ = df[clu_to_join].groupby(level=1, axis=1).sum(
            min_count=1)
        # crea el nombre de las columnas
        df_.columns = pd.MultiIndex.from_product(
            [['C_'], ['vx', 'vy']])
        ...
        # cambia el nombre de la nueva columna de agrupamientos, de C_ al primer de la lista de clusters que se paso
        df.rename({'C_': clu_to_join[0]}, axis=1,
                  inplace=True)
    else:
...
# la imagen.
# Todo: Cambiar la condicion para cambiar cero por nan, ahorita busca el string "0." pero no es lo ideal,
# que pasa si es "0" o "0e+0"?
# Todo: revisar bien el formato de las paths [1:-1]?


def create_mc_plot(fign, trans_matrix,
                   filename='graph_mc.jpg', marker_size=10,
                   node_fontsize=14, line_width=1,
                   x_width=20, y_width=10, fig_size=(10, 5),
                   replace_zero_to_nan=True, as_pct=True,
                   layout='auto',
    ...
    :param fign: numero de figura
    :param layout: opciones de layout de matlab para graphplot. No todas estan implementadas: UseGravity, auto, WeightEffect, layered, circle.
    ...
    f.write('mc.StateNames = ' + state_names)
    f.write(
        'p = graphplot(mc,"ColorEdges",true,"LabelEdges",true);\n')
    # layout del grafico
    if layout == 'auto':
        ...
        if trans_matrix.eq(
                0).values.any() or trans_matrix.isnull().values.any():
            f.write(
    ...
    if result.stderr:
        raise subprocess.CalledProcessError(
            returncode=result.returncode, cmd=result.args,
...
def join_clusters(df, clu_to_join, clu_to_del=None):
    ...
    Combinas las columnas especificadas en clu_to_join en una sola, es decir, combina clusters, df = dfclvv. El orden
     ...
     Tambien es important usar doble corchete.
    ...
    :param clu_to_join: clusters a combinar en pares. [[1,2],[7,9] une el primero el cluster 1 y 2 y despues une el
    cluster 7,9.
    ...
    if len(np.array(clu_to_join,
                    dtype='object').shape) != 1:
        logging.error(
            'Formato de lista de grupos erronea. Sigue el formato [[a,b...n]]?')
        return
    ...
    if df.columns.nlevels == 1:
        for cl in clu_to_join:
            ...
            # se vuelven cero
            df = df.assign(
                C_=df[cl].sum(1, min_count=1)).drop(cl,
                                                    axis=1)
            # cambiar el nombre de columna temporal C_ a el primer valor de la lista de columnas a combinar
        ...
        for cl in clu_to_join:
            ...
            # groupby agrupa todos las columnas de los clusters seleccionados en una, sum suma esas columnas y min_count
            # hace que n + NaN sea n. Sin min_count creo que la suma da cero
            df_ = df[cl].groupby(level=1, axis=1).sum(
                min_count=1)
            # crea el nombre de las columnas
            df_.columns = pd.MultiIndex.from_product(
                [['C_'], ['vx', 'vy']])
        ...
        logging.error(
            'Error: Formato de Dataframe no soportado.')
    return df


# %% md
# Matriz de transicion a partir de dataframe
# %%
def trans_matrix_from_df(df, mat_mode='prob'):
    ...
    """
    # matriz de transicion del tamanio del numero de  clusters
    ...
    for row in range(len(df) - 1):
        ...
        if not df.iloc[row].isnull().all() and not df.iloc[
            row + 1].isnull().all():
            # para asegurarse de que no hay filas con cero?
            if df.iloc[row].to_numpy().nonzero()[0].shape[
                0] > 0 and \
                    df.iloc[row + 1].to_numpy().nonzero()[
                        0].shape[0] > 0:
                if df.iloc[row][np.argwhere(
                    ~np.isnan(df.iloc[row].values) == True)[
                    0][0]] > 0:
                    i = np.argwhere(~np.isnan(
                        df.iloc[row].values) == True)[0][
                        0]  # donde estoy (que estado)
                    j = np.argwhere(~np.isnan(
                        df.iloc[row + 1].values) == True)[
                        0][
    ...
    mprob = df_mat_tra.apply(
        lambda row: row.astype(float) / row.sum() * 100,
        axis=1)
...
# %% md
# crear dataframe de datos agrupados en clusters (dfclvv, no se usa la funcion kmeans)
# %%
...
def create_clustered_data(df_to_clus, klabels):
    ...
    for i in range(n_clusters):
        dfclvv = pd.concat(
            [dfclvv, df_to_clus.vwind[klabels == clord[i]]],
